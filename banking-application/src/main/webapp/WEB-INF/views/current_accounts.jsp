...
<%   List<CurrentAccount> currentAccounts = (List<CurrentAccount>)session.getAttribute("currentAccounts"); %>
 ...
 <table >
  ...
  <tbody>
  	<c:forEach items="${currentAccounts}" var="c">
		
		<tr>
		<td> <c:out value="${c.getAccountName()}"/> 
		</td>
		<td> <c:out value="${c.getAccountNumber()}"/> </td>
		<td> <c:out value="${c.getBalance()}"/> </td>
		<td> <c:out value="${c.getDateCreated()}"/> </td>
		<td>  <form action="/updateAccounts" method="POST">
   
			    <input type = "hidden" name="<%= "accountNumber" %>" value="${c.getAccountNumber()}" />
			    <input type = "hidden" name="<%= "accountType" %>" value="Cur" />
			    <input type="submit" class=" btn btn-primary registerbtn"  value="Update"/>
